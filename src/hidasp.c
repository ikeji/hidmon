...
int hidWriteBuffer(char *buf, int len)
{
    ...
    if( report_id == 0) {
        ...
        return hidWrite(hHID, (char *)buf, length, report_id);
    }
}
...
int hidCommand(int cmd,int arg1,int arg2,int arg3)
{
    ...
    return hidWrite(hHID, (char *)buf, REPORT_LENGTH1, REPORT_ID1);
}
...
int hidPeekMem(int addr)
{
    ...
    hidWrite(hHID, (char *)buf, REPORT_LENGTH1, REPORT_ID1);
...
//  Sleep(100);
    ...
    for (i = 0; i < CHECK_COUNT; i++) {
        ...
        r = hidRead(hHID, (char *)rd_data ,REPORT_LENGTH1, REPORT_ID1);
#if DEBUG
