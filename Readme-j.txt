...
--------------------------【重要】--------------------------
複数の HIDaspx を利用する場合には、シリアル番号設定は非常に重要です。

複数の HIDaspx が同じ場所にあれば、同じパソコンにセットする可能性は十分に
考えら、同一の VID/PID, シリアル番号の HIDaspx が一台のパソコンに複数個が
セットされた瞬間、「Windows が再起動する」可能性があります。

異なるシリアル番号を設定することでこの問題は解決でき、同時に使い分けも可
能になります。複数台の HIDaspx を利用する方は、ぜひ適正なシリアル番号を設
定してご利用ください。
--------------------------【重要】--------------------------
...
2008-1028 版以降、「-v adr= 」を指定することで、任意のアドレスに対するシリアル情
報を生成でき、また、複数のシリアル番号も瞬時に生成可能です。
...
のように使います。実行すると、0001 のシリアルデータの HEX ファイル（_0001.hex）を
生成できます。
...
genserial.awk はシリアル番号だけの HEX ファイルを生成しますが、hidspx 用
の GUI ツールは、単一の HEX ファイルのみに対応であり、シリアル情報の書き
込みには利用できません。そこで、2010年 2月 10日の版から、addserial.awk を
追加しました。使い方は genserial.awk と同じですが、HIDaspx 用の HEX ファ
イルを含む HEX を生成できます。
...
【重要】
実習などで、大量に HIDaspx を作成する場合には、必ずユニークなシリアル番号
を追加した HEX ファイルを書き込んでください。同一のシリアル番号の利用は
OS の誤動作を引き起こす場合があります。
