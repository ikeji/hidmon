...
■2010-02-08
	(1) HIDaspxのシリアル番号の省略（ワイルドカード指定）を可能にしました。
	    hidspx にて、HIDaspx のシリアル番号の自動認識できる仕様に変更しました。
	    複数の HIDaspx が接続されている場合には、最初に認識されたものが操作対
	    象になります。複数の HIDaspx を選択して指定するには、例 3 の様に、希
	    望する装置のシリアル番号を指定します。シリアル番号は、例 1 の方法で調
	    査できます。
  例1）
